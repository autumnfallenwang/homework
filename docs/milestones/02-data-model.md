...
- **Column names are snake_case** (`child_id`, `hw_date`) via `casing: "snake_case"` on both `drizzle.config.ts` and the `drizzle()` client. (Initially generated camelCase from the JS property names; switched in M03 — without an explicit `casing`, drizzle-orm's runtime `ON CONFLICT` target emitted snake_case while the DDL was camelCase, so upserts never matched the unique constraint. snake_case everywhere is consistent, conventional for Postgres, and matches the source SQLite schema.) `getTableConfig` still reports the JS-property names, so the schema unit test asserts those.
- **FK delete behavior refined in M03** (so `DELETE /children/:id` works): the source left `grades/assignments/standards.classId` and `standards.parentId` with no ON DELETE rule, which blocked deleting a child (its `classes` cascade-delete was refused while child-scoped rows still referenced them). Now `*.classId` → `ON DELETE set null` (rows are also fetch-run-scoped, so they cascade-delete via `fetchRunId` anyway; nulling just survives a lone class removal) and `standards.parentId` → `ON DELETE cascade` (self-ref tree). Migration regenerated; 8 cascade + 3 set-null FKs.
